...
def setup_logger():
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(levelname)s - [%(request_id)s] - [%(user_ip)s] -"
        "[%(cust_id)s] - %(message)s"
    )
    # Creates daily log file at midnight with backup logs for last 7 days
    timed_handler = TimedRotatingFileHandler(app.config["LOG_FILE"],
                                             when='midnight', backupCount=7)
    timed_handler.setLevel(app.config["LOG_LEVEL"])
    timed_handler.setFormatter(formatter)
    logger.addHandler(timed_handler)

    # Rotating when log file reaches 50MB and keeping at most 3 backup files
    rotating_handler = RotatingFileHandler(app.config["LOG_FILE"],
...
flask_logger = FlaskLogger(setup_logger(), extra={"user_ip": "",
                                                  "request_id": "",
                                                  "cust_id": ""})
...
@app.before_request
def before_request():
    # Generate a unique request_id, user_ip and cust_id to FlaskLogger extra
...
def customer_id_detailed():
    ...
    cust_device_dict = database.get_customer_devices(engine, customer_id,
...
def customer_id_device_id_only():
    customer_id = request.args.get('customer-id')
    flask_logger.info('User access the customer id')
...
@app.route('/')
def index():
    flask_logger.info('User access the index page')
    return 'Welcome to the Orion Customer Dashboard API'


if __name__ == '__main__':
    app.run(debug=True, port=8444, host='0.0.0.0')
